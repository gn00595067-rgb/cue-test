...
# 1. 頁面設定 & 自動載入
# =========================================================
st.set_page_config(layout="wide", page_title="Cue Sheet Pro v69.1 (GPT Restore)")
...
# =========================================================
# 2. Excel 轉 PDF (GPT 原版邏輯: LibreOffice)
# =========================================================
...
def xlsx_bytes_to_pdf_bytes(xlsx_bytes: bytes):
    ...
    if not soffice: return None, "Fail", "無可用的 LibreOffice 轉檔引擎"
...
def generate_excel_from_template(format_type, start_dt, end_dt, client_name, product_display_str, rows, remarks_list, template_bytes):
    ...
    for i, (m_key, start_row_orig) in enumerate(sec_order):
        ...
        for idx, r_data in enumerate(data):
            ...
            if format_type == "Dongwu":
                ...
                safe_write(ws, f"{cols['proj_price']}{curr_row}", r_data["pkg_display_val"])
    ...
    if total_cell:
        ...
        # [Fix] Total 填入全省打包價 (Value Anchor)
        total_pkg = 0
        processed_nat = set()
        for r in rows:
            if r.get("is_pkg_member"):
                if r["media_type"] not in processed_nat:
                    # 全省 List / Std * Factor * Spots
                    db = PRICING_DB[r["media_type"]]
                    factor = get_sec_factor(r["media_type"], r["seconds"])
                    nat_list = db["全省"][0] # List
                    nat_rate = int((nat_list / db["Std_Spots"]) * factor)
                    total_pkg += nat_rate * r["spots"]
                    processed_nat.add(r["media_type"])
            else:
                total_pkg += r["pkg_display_val"] if isinstance(r["pkg_display_val"], int) else 0
...
# 4. 簡易 HTML 預覽
# =========================================================
def generate_html_preview(rows, days_cnt, start_dt, end_dt, c_name, p_display, format_type):
    # 這裡只做簡單預覽，讓使用者知道產出什麼
    # 為了不讓程式碼過長，省略複雜樣式，重點在 Excel 下載
    return f"<div>Preview Generated (Rows: {len(rows)})</div>"

# =========================================================
# 5. UI Main
# =========================================================
st.title("📺 媒體 Cue 表生成器 (v69.1)")
            ...
            if m in ["全家廣播", "新鮮視"]:
                ...
                for i, r in enumerate(display_regs):
                    ...
                    rows.append({
                        ...
                        "is_pkg_member": cfg["is_national"] # 用於 Total 計算判斷
                    })
...
if rows and template_bytes:
    try:
        xlsx = generate_excel_from_template(format_type, start_date, end_date, client_name, p_str, rows, rem, template_bytes)
        if xlsx:
            st.download_button("下載 Excel (OpenPyXL Render)", xlsx, f"Cue_{safe_filename(client_name)}.xlsx")
            
            # PDF 部分使用 LibreOffice (如果雲端有安裝的話)
            ...
            if pdf_bytes:
                ...
                st.warning(f"PDF 轉檔失敗 (雲端無 LibreOffice): {err}")
    except Exception as e:
