...
# 1. 頁面設定 & 模板載入
# =========================================================
st.set_page_config(layout="wide", page_title="Cue Sheet Pro v65.0 (Dynamic Block Renderer)")
...
def load_default_template():
    ...
    if GOOGLE_DRIVE_FILE_ID:
        ...
        try:
            ...
            if r.status_code == 200:
                if b"<!DOCTYPE html>" in r.content[:800]:
                    status_msg.append("⚠️ Drive 下載權限受限（可能需要登入或分享權限）")
                else:
    ...
    if os.path.exists(DEFAULT_FILENAME):
        try:
            ...
            status_msg.append(f"⚠️ Local 模板讀取失敗: {e}")

    return None, None, status_msg

# =========================================================
# 2. Excel -> PDF（擬真優先：本機 Excel COM -> LibreOffice -> HTML Fallback）
# =========================================================
...
def xlsx_bytes_to_pdf_bytes(xlsx_bytes: bytes):
    ...
    # 2) LibreOffice（雲端/Server 首選，擬真度高）
    ...
    if soffice:
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                p = subprocess.run(
                    [soffice, "--headless", "--nologo", "--nolockcheck", "--convert-to", "pdf", "--outdir", tmp, xlsx_path],
                    capture_output=True,
                    timeout=90,
                )

                # LibreOffice 可能輸出不同檔名
                pdf_path = os.path.join(tmp, "cue.pdf")
                ...
                msg = (err.strip() or out.strip() or "LibreOffice 轉檔無輸出")
                return None, "Fail", msg
        except Exception as e:
            return None, "Fail", str(e)

    return None, "Fail", "此環境未偵測到 LibreOffice/soffice（可用 HTML Fallback 產 PDF）"

# =========================================================
# 3. HTML -> PDF Fallback（WeasyPrint）
# =========================================================
...
def html_to_pdf_fallback(html_str, font_b64):
    ...
    css_str = """
    @page { size: A4 landscape; margin: 0.45cm; }
    body { font-family: 'NotoSansTC', Arial, sans-serif !important; font-size: 8pt; }
    table { width: 100%; border-collapse: collapse; }
    th, td { border: 0.5pt solid #555; padding: 2px 3px; text-align: center; vertical-align: middle; white-space: nowrap; }
    ...
    .right { text-align: right; padding-right: 6px; font-family: Consolas, 'Courier New', monospace; }
    thead { display: table-header-group; }
    tr { page-break-inside: avoid; }
    """

    if font_b64:
        css_str = f"@font-face {{ font-family: 'NotoSansTC'; src: url(data:font/ttf;base64,{font_b64}) format('truetype'); }}\n" + css_str
...
# 讓 Dongwu anchors 搜尋更穩（家樂福常不在 B 欄）
SHEET_META["Dongwu"]["anchor_search_cols"] = ["B", "C", "D"]
# Dongwu 模板：家樂福 anchor 可能是標題列，資料從下一列開始（視你的模板可調 0/1）
SHEET_META["Dongwu"]["carrefour_data_offset"] = 1

# =========================================================
# 5.x Excel 動態表格渲染：Block Renderer（可自動增刪列/避免空列/維持模板擬真）
# =========================================================
...
def render_block_dongwu(ws, meta, format_type, blk, sec_groups):
    ...
    # 超關鍵：解除區塊內舊 merge（含 G，避免 rate 寫入錯亂）
    unmerge_overlaps_by_cols(ws, s, new_end, ["B", "E", "F", "G", "H"])
    ...
    # 逐列寫入（C/D/G + schedule + total spots）
    for i, line in enumerate(lines):
        r = s + i
        item = line["item"]

        safe_write(ws, f"{cols['location']}{r}", region_display(item["region"]))
        prog_val = item.get("program_num", parse_count_to_int(item.get("program", 0)))
    ...
    for g_start, g_end, g in group_ranges:
        ...
        # Package (H)：顯示該 seconds 群的總 package
...
def render_block_shenghuo(ws, meta, format_type, blk, sec_groups):
    ...
    # Shenghuo 通常不 merge，但保險：解除 station 欄的舊 merge（若模板有）
    unmerge_overlaps_by_cols(ws, s, new_end, ["B", "C", "D", "E", "F", cols.get("proj_price", "AF")])
...
def generate_excel_from_template(format_type, start_dt, end_dt, client_name, product_display_str, rows, remarks_list, template_bytes):
    ...
    # 先解除整個 data area 的 merge，避免 delete/insert rows 後 Total/Footer 被吞
    data_start = min(b["start"] for b in blocks)
    data_end = total_row_orig - 1
    if format_type == "Dongwu":
        unmerge_overlaps_by_cols(ws, data_start, data_end, ["B", "E", "F", "G", "H"])

    # 分群資料
    media_groups = build_groups(rows)

    # 由下往上渲染（避免 row index 連鎖位移）
    ...
    total_pos = find_cell_exact_in_ws(ws, meta["total_label"])
    if not total_pos:
        raise ValueError("渲染後找不到 Total（可能模板 Total 文字被改或被公式/合併影響）")
    total_row = total_pos[0]
...
def generate_html_preview(rows, days_cnt, start_dt, end_dt, c_name, p_display, format_type, remarks):
    ...
    # CSS（更擬真 + sticky header）
    css = """
    <style>
      .excel-wrap { background:#fff; border:1px solid #d0d0d0; padding:12px; box-shadow:0 2px 8px rgba(0,0,0,.08); }
      table.excel { border-collapse:collapse; font-size:12px; width:max-content; min-width:100%; }
      table.excel th, table.excel td { border:1px solid #777; padding:3px 6px; height:22px; text-align:center; vertical-align:middle; white-space:nowrap; }
      table.excel thead th { position: sticky; top: 0; z-index: 5; }
      .bg-dw-head { background:#4472C4; color:#fff; font-weight:700; }
      .bg-sh-head { background:#BDD7EE; color:#000; font-weight:700; }
      .bg-weekend { background:#FFD966; color:#000; font-weight:700; }
      .bg-total { background:#FFF2CC; font-weight:700; }
      .left { text-align:left !important; padding-left:8px !important; }
      .right { text-align:right !important; padding-right:8px !important; font-family:Consolas, "Courier New", monospace; }
      .col-day { min-width:26px; }
      .col-station { min-width:110px; }
      .col-loc { min-width:95px; }
      .col-prog { min-width:70px; }
      .col-daypart { min-width:95px; }
      .col-size { min-width:70px; }
      .col-rate { min-width:90px; }
      .col-pkg { min-width:110px; }
      .remarks { margin-top:10px; font-size:12px; line-height:1.45; }
    </style>
    """
    ...
    for _ in range(eff_days):
        ...
        if format_type == "Shenghuo":
            bg = header_cls
        date_th1 += f"<th class='{bg} col-day'>{curr.day}</th>"
        date_th2 += f"<th class='{bg} col-day'>{weekdays[wd]}</th>"
        curr += timedelta(days=1)

    if format_type == "Dongwu":
        cols_def = ["Station", "Location", "Program", "Day-part", "Size", "rate<br>(List)", "Package<br>(List)"]
        col_classes = ["col-station","col-loc","col-prog","col-daypart","col-size","col-rate","col-pkg"]
    else:
        cols_def = ["頻道", "播出地區", "播出店數", "播出時間", "秒數<br>規格", "專案價"]
        col_classes = ["col-station","col-loc","col-prog","col-daypart","col-size","col-pkg"]

    th_fixed = ""
    for c, cls in zip(cols_def, col_classes):
        th_fixed += f"<th class='{header_cls} {cls}' rowspan='2'>{c}</th>"

    # 排序
    def region_idx(r):
        return REGIONS_ORDER.index(r) if r in REGIONS_ORDER else 999

    rows_sorted = sorted(
        rows,
        key=lambda x: (media_order.get(x["media_type"], 99), x["seconds"], region_idx(x["region"])),
    )

    # media rowspans（Dongwu station 會 rowspan）
    media_counts = {}
    ...
    for r in rows_sorted:
        ...
        if format_type == "Dongwu":
            if not media_printed[m]:
                ...
                display_name = (
                    "全家便利商店<br>通路廣播廣告" if m == "全家廣播"
                    else "全家便利商店<br>新鮮視廣告" if m == "新鮮視"
                    else "家樂福"
                )
                tbody += f"<td class='left col-station' rowspan='{rowspan}'>{display_name}</td>"
                media_printed[m] = True
        else:
            display_name = (
                "全家便利商店<br>廣播通路廣告" if m == "全家廣播"
                else "全家便利商店<br>新鮮視廣告" if m == "新鮮視"
                else "家樂福"
            )
            tbody += f"<td class='left col-station'>{display_name}</td>"

        tbody += f"<td class='col-loc'>{html_escape(region_display(r['region']))}</td>"
        tbody += f"<td class='right col-prog'>{html_escape(r.get('program_num',''))}</td>"
        tbody += f"<td class='col-daypart'>{html_escape(r.get('daypart',''))}</td>"

        if format_type == "Dongwu":
            sec_txt = f"{r['seconds']}秒" if m == "家樂福" else f"{r['seconds']}"
            tbody += f"<td class='col-size'>{html_escape(sec_txt)}</td>"
            rate = f"{r['rate_list']:,}" if isinstance(r.get("rate_list"), int) else html_escape(r.get("rate_list"))
            pkg = f"{r['pkg_display_val']:,}" if isinstance(r.get("pkg_display_val"), int) else html_escape(r.get("pkg_display_val"))
            tbody += f"<td class='right col-rate'>{rate}</td>"
            tbody += f"<td class='right col-pkg'>{pkg}</td>"
        else:
        ...
        for d in r.get("schedule", [])[:eff_days]:
            tbody += f"<td class='col-day'>{int(d) if isinstance(d,(int,float)) else html_escape(d)}</td>"

        tbody += f"<td class='bg-total col-day'>{sum(r.get('schedule', [])[:eff_days])}</td>"
        tbody += "</tr>"
    ...
    for r in rows:
        ...
        for i in range(min(eff_days, len(sch))):
            totals[i] += sch[i]
    total_pkg = sum([x["pkg_display_val"] for x in rows if isinstance(x.get("pkg_display_val"), int)])

    colspan = 5 if format_type == "Dongwu" else 4
    if format_type == "Dongwu":
        tfoot = f"<tr class='bg-total'><td colspan='{colspan}' class='left'>Total</td><td></td><td class='right'>{total_pkg:,}</td>"
    else:
        tfoot = f"<tr class='bg-total'><td colspan='{colspan}' class='left'>Total</td><td class='right'>{total_pkg:,}</td>"
    for t in totals:
        tfoot += f"<td class='col-day'>{t}</td>"
    tfoot += f"<td class='col-day'>{sum(totals)}</td></tr>"

    header_info = f"""
    <div style="margin-bottom:10px;">
      <b>客戶：</b>{html_escape(c_name)} &nbsp;&nbsp;
      <b>產品：</b>{html_escape(p_display)}<br>
      <span style="color:#666;">走期：{html_escape(start_dt)} ~ {html_escape(end_dt)}</span>
    </div>
    """
    ...
    <div class="excel-wrap">
      ...
      <div style="overflow:auto;">
        <table class="excel">
          <thead>
            <tr>
              {th_fixed}{date_th1}
              <th class="{header_cls} col-day" rowspan="2">檔次</th>
            </tr>
            <tr>{date_th2}</tr>
          </thead>
          <tbody>
      ...
      <div class="remarks"><b>Remarks：</b><br>{remarks_html}</div>
    </div>
    """
...
# 7. UI
# =========================================================
st.title("📺 媒體 Cue 表生成器 (v65.0: Dynamic Block Renderer)")
...
def on_slider_change(changed_key):
    ...
    val = int(st.session_state.get(changed_key, 0))
    val = max(0, min(100, val))
    st.session_state[changed_key] = val
    ...
    sum_others = int(st.session_state.get(k1, 0)) + int(st.session_state.get(k2, 0))
    if sum_others <= 0:
        st.session_state[k1] = rem // 2
        st.session_state[k2] = rem - st.session_state[k1]
        return

    ratio = float(st.session_state.get(k1, 0)) / float(sum_others)
...
if is_rad:
    with m1:
        ...
        config["全家廣播"] = {
            ...
            "regions": regs,
            "seconds": sorted(secs),
            "share": int(st.session_state.rad_share),
...
if is_fv:
    with m2:
        ...
        config["新鮮視"] = {
            ...
            "regions": regs,
            "seconds": sorted(secs),
            "share": int(st.session_state.fv_share),
            "sec_shares": sec_shares,
        }
...
if is_cf:
    with m3:
        ...
        config["家樂福"] = {
            "regions": ["全省"],
            "seconds": sorted(secs),
            "share": int(st.session_state.cf_share),
            "sec_shares": sec_shares,
        }
...
if config:
    for m, cfg in config.items():
        m_budget = float(total_budget_input) * (float(cfg["share"]) / 100.0)
        for sec, sec_pct in cfg["sec_shares"].items():
            s_budget = m_budget * (float(sec_pct) / 100.0)
            if s_budget <= 0:
                continue
            ...
            if m in ["全家廣播", "新鮮視"]:
                db = PRICING_DB[m]
                calc_regs = REGIONS_ORDER if cfg.get("is_national") else cfg["regions"]
                display_regs = REGIONS_ORDER if cfg.get("is_national") else cfg["regions"]
                ...
                for r in display_regs:
                    ...
                    is_start = bool(cfg.get("is_national") and r == "北區")
                    rows.append(
                        {
                            "media_type": m,
                            "region": r,
                            ...
                            "is_pkg_start": is_start,
                            "is_pkg_member": bool(cfg.get("is_national")),
                        }
                    )
...
p_str = (
    f"{'、'.join([f'{s}秒' for s in sorted(list(set(r['seconds'] for r in rows)))])} {product_name}"
    if rows
    else ""
)
...
if rows:
    ...
    if template_bytes:
        try:
            ...
            out_name = safe_filename(f"Cue_{client_name}")
            st.download_button("下載 Excel", xlsx, f"{out_name}.xlsx")

            # PDF 優先：Excel/LibreOffice
            pdf_bytes, method, err = xlsx_bytes_to_pdf_bytes(xlsx)
            if pdf_bytes:
                st.download_button(f"下載 PDF ({method})", pdf_bytes, f"{out_name}.pdf")
            else:
                st.warning(f"Excel 轉 PDF 失敗（{method}: {err}），改用 HTML Fallback 產 PDF（擬真度較低）")
                font_b64 = load_font_base64()
                pdf_bytes, err2 = html_to_pdf_fallback(html, font_b64)
                if pdf_bytes:
                    st.download_button("下載 PDF (Fallback)", pdf_bytes, f"{out_name}.pdf")
                else:
    ...
    st.info("請先選擇媒體與秒數/區域設定，系統才會生成 rows 與 Cue 表。")
